...
- **Endpoint filter axis.** Add `allow_endpoint_in`/`block_endpoint_in`/`allow_endpoint_out`/`block_endpoint_out` alongside the existing msgid / src_sys / src_comp axes, keyed by endpoint id (or name). Lets a config say "only forward frames from `serial0` to `udpc0`" without rewriting routing as filters on every other axis. Blocklist still wins over allowlist on overlap, same as the rest of `Filters::apply`.
