...
- [ ] **Use-case examples** under `examples/`. Each is a runnable TOML config (and/or CLI args) with a short README walking through the scenario, the chosen endpoints, and which filters/flags do the work. Cover at minimum:
  ...
  4. **Sniffer / diagnostic tap** — `?sniffer=true` on a dedicated `udps:` or `tcps:` so a logger or `tcpdump`-style tool sees every routed frame regardless of target.
  5. **NAT traversal via VPS rendezvous** — vehicle-side RMR dials out (`tcpc:rendezvous.example.com:5760`) to a VPS-side RMR running `tcps:0.0.0.0:5760` + `tcps:0.0.0.0:5761`; remote GCSs connect to the second port. Shows RMR as a connection broker for vehicles behind carrier-grade NAT.
