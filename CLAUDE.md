...
- Filter cost: each `Allow*`/`Block*` list is small (typically <20 entries), stored as a `SmallVec<u32, 8>` and scanned linearly for membership. At n ≤ 20 a branch-predictable linear scan over contiguous u32s is faster than binary search (better cache behaviour, no pivot mispredicts) and avoids the need to keep the list sorted on insert. No allocation in the common case.
...
- [ ] `udps:` — bind, multi-peer learning, idle reap (configurable, default 60s); dual-stack on `[::]` via `socket2` setting `IPV6_V6ONLY=0` on both Windows and Linux for parity
- [ ] `udpc:` — send to configured `host:port` initially; latch onto inbound source address only when the source IP matches the resolved configured host (port may differ); drop + count packets from other IPs at ingress; revert to configured address after `latch_idle_secs` (default 30s) of silence from the latched peer, re-resolving DNS on revert; DNS re-resolve on every send burst after a `send_to` error
- [ ] Round-trip integration test: two `udps:` instances exchange frames
- [ ] `udpc:` integration test: configured peer sends from a different ephemeral port (same IP) → router latches and replies to the latched addr; packet from an unrelated IP is dropped + counted; after `latch_idle_secs` of silence the router reverts to the configured `host:port`

### Phase 3 — TCP endpoints

- [ ] `tcps:` — accept loop, per-client task, **each accepted client is its own routing endpoint** (own learn-set, own stats, filter inheritance by reference from the listener); `SO_KEEPALIVE` on (OS defaults), `TCP_NODELAY`, dual-stack on `[::]` via `socket2` (`IPV6_V6ONLY=0`); removal on disconnect without killing the router (`mavlink-router` had this bug)
- [ ] `tcpc:` — dial, capped-exponential backoff (250 ms → 30 s, ±20% jitter), `TCP_NODELAY`, `SO_KEEPALIVE` on (OS defaults), DNS re-resolve on each reconnect
- [ ] Initial bind/dial failure path: `tcps:` that can't bind enters the same backoff loop as `tcpc:`; router stays up and logs at WARN per attempt
- [ ] Integration test: kill a TCP client mid-stream, verify router stays up and reconnects within the backoff bound
- [ ] Integration test: start a `tcps:` on a port already in use, free the port, verify the listener picks it up without restart
...
| 1     | Framer unit + property tests; capture replay; `build.rs` XML parse + `crc_extra` computation tests; `<include>` resolution and cycle detection. |
| 2     | UDP round-trip integration (two `udps:` instances exchange frames); peer idle-reap test. |
| 3     | TCP-client mid-stream disconnect → router survives; TCP-server accepts many; client kill → reconnect within N seconds. |
...
- **`twox-hash` feature pin:** dependency declared as `twox-hash = { version = "2", default-features = false, features = ["xxh3"] }`. xxh3-64 is behind the `xxh3` feature in twox-hash 2.x; the default features pull in legacy variants we don't need.
